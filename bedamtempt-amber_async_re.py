...
class OpenCLContextSDM(OpenCLContext):
    ...
    def  _openmm_worker_body(self):
        ...
        if implicitsolvent is None or 'None' in implicitsolvent:
            self.system = self.dms.createSystem(nonbondedMethod=NoCutoff, implicitSolvent = None)
        elif implicitsolvent == 'AGBNP':
            self.system = self.dms.createSystem(nonbondedMethod=NoCutoff, implicitSolvent = 'AGBNP')
        else:
