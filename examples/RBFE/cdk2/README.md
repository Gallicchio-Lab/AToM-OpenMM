...
The `run.sh` launch script is launched for each ligand pair. `run.sh` runs the `scripts/run-atm.py` application, which is the main workhorse of the workflow (run `python scripts/run-atm.py` without arguments to see the arguments it accepts). The `run-atm.py` application performs the following tasks:
...
- It sets the simulation control parameters specific to the ligand pair, such as the indices of the ligands and other things. It saves the specific settings, along with the default settings, in a `.yaml` file in the simulation directory.
- It runs the `rbfe_structprep` application to equilibrate and anneal the system.
- It runs the `rbfe_production` application to run the alchemical Hamiltonian Replica Exchange simulation to collect the perturbation energy data. The calculations in this proposal use the variable-displacement algorithm, which (alchemically) swaps the positions of the two ligands based on the current distance between the anchor atoms of the ligands. Additionally, the bound ligand is confined to a spherical binding-site region defined with respect to an internal coordinate frame of the receptor. This allows the complex and the unbound ligand to move freely and independently in the simulation box. A custom short-ranged repulsion potential prevents the unbound ligand to interact with the receptor. 
...
```
cd $HOME/AToM-OpenMM/examples/RBFE/cdk2/complexes/cdk2-H1Q-H1R
vmd -f cdk2-H1Q-H1R_0.pdb `/bin/ls -v r*/*xtc` -e vmd.in
```
