...
- It imports the shared `make_pp_indexes()` helper from `atom_openmm.utils.AtomUtils` and applies it to the generated system topology to determine the peptide-specific ATM keys, including the peptide atom lists, the common-variable atom split, the attachment atoms, and `ALIGN_LIGAND1_REF_ATOMS` / `ALIGN_LIGAND2_REF_ATOMS` from the mutation-residue `CA`, `N`, and `C` atoms. The standalone `scripts/make_pp_indexes.py` wrapper remains available for ad hoc use on prepared PDB files.
