...
for l in `seq 0 $n` ; do
    ...
    if [ ${#ligs[@]} -ne 3 ] ; then
        echo "Error: expected 3 fields in ligand pair entry, got '${ligpair}'"
        exit 1
    fi

    echo "Processing ligand pair $lig1 $lig2 mutation residue $mutres ..."
    ...
    sed "s#<JOBNAME>#${jobname}#g ; s#<WORKDIR>#${jobwdir}# ; s#<SCRIPTS_DIR>#${scripts_dir}# ; s#<CONDADIR>#${CONDA_PREFIX}# ;  s#<CONDAENV>#${CONDA_DEFAULT_ENV}# ; s#<RCPT>#${receptor}# ; s#<LIG1>#${lig1}# ; s#<LIG2>#${lig2}# ; s#<MUTRES>#${mutres}# " < ${scripts_dir}/run_template.sh > ${jobwdir}/run.sh
