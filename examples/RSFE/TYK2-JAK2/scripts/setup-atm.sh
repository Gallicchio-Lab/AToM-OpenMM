...
for l in `seq 0 $n` ; do
    ...
    jobname=${basename}-${lig1}-${lig2}
    mkdir -p ${work_dir}/complexes/${jobname} || exit 1
    cd ${work_dir}/complexes/${jobname}    || exit 1

    #files of receptor and ligands
    rcpt1pdb=${work_dir}/receptor/${receptor1}.pdb
    rcpt2pdb=${work_dir}/receptor/${receptor2}.pdb
    ...
    python $AToM_dir/make_atm_system_from_rcpt_lig.py --receptorinFile "${rcpt1pdb}" --receptor2inFile "${rcpt2pdb}"  --LIG1SDFinFile "${lig1sdf}" --LIG2SDFinFile "${lig2sdf}" --displacement "${displs}" --systemXMLoutFile "${jobname}_sys.xml" --systemPDBoutFile "${jobname}.pdb" --hmass 1.5 --forcefieldJSONCachefile "${work_dir}/ligands/ffdb.json" || exit 1
...
#prepare prep script
sed "s#<RECEPTOR>#${basename}# ; s#<LIGPAIRS>#${ligpreppairs}# ; s#<ASYNCRE_DIR>#${AToM_dir}#g " < ${work_dir}/scripts/prep_template.sh > ${work_dir}/complexes/prep.sh

#prepare free energy calculation script
sed "s#<RECEPTOR>#${basename}# ; s#<LIGPAIRS>#${ligpreppairs}# " < ${work_dir}/scripts/free_energies_template.sh > ${work_dir}/complexes/free_energies.sh
